# Agent Instructions: Azure Architecture Diagram Generation (Linux/WSL)
...
### Python Environment
- **Location**: `/mnt/d/workspace/architecture-diagrams-generator/diagram/venv`
...
cd "/mnt/d/workspace/architecture-diagrams-generator/diagram"
python3 -m venv venv
source venv/bin/activate

# 2. Install system dependencies for pygraphviz
sudo apt update
sudo apt install -y build-essential graphviz graphviz-dev pkg-config python3-dev
...
- **Location**: `/usr/bin` (dot command)
- **Version**: Installed via apt (typically latest available)
- **Critical**: Should be in PATH by default; if not, ensure `/usr/bin` is in PATH
...
├── instructions.md                 # Diagram requirements
├── architecture-diagram/           # Documentation & instructions
│   ├── agent.md                    # Windows instructions
│   ├── agent-linux.md              # Linux/WSL instructions (THIS FILE)
│   └── requirements.txt            # Python dependencies
└── diagram/                        # Working directory for generation
    ├── venv/                       # Python virtual environment
    ├── diagrams/                   # Output directory
    │   ├── *.png                   # PNG image outputs
    │   ├── *.dot                   # GraphViz DOT source files
    │   └── *.drawio                # Editable draw.io files
    └── contoso_architecture.py     # Contoso diagram generation script
```
...
```python
subprocess.run([
    "graphviz2drawio", # Ensure this is in PATH or use absolute path (e.g., ./venv/bin/graphviz2drawio)
    "diagrams/<name>.dot", 
    "-o", 
...
from diagrams.azure.network import (
    ...
    PrivateEndpoint, DNSPrivateZones, Firewall  # Use 'Firewall' for Azure Firewall
)
...
from diagrams.azure.general import Usericon  # Use this for 'Users' icon
```
...
### Import Errors
**Error**: `cannot import name 'PublicIPAddresses'` or `cannot import name 'Firewalls'` or `AzureFirewalls'`

**Solution**: Check exact class name (case-sensitive):
- Correct: `PublicIpAddresses`, `Firewall`
- Correct: `PublicIpAddresses`, `Firewall`
- Wrong: `PublicIPAddresses`, `Firewalls`, `AzureFirewalls`

### User Icon Not Found
**Error**: `ImportError: cannot import name 'Users'`

**Solution**: The class name is `Usericon`.
```python
...
### Compilation Errors for pygraphviz
**Error**: `Python.h: No such file or directory` or GCC not found

**Solution**: Install build dependencies:
```bash
sudo apt update
sudo apt install -y build-essential python3-dev graphviz-dev pkg-config
```
...
### 2. Contoso Architecture (Latest Agent Run)
- **File**: `diagram/contoso_architecture.py`
- **Scenario**: Full Contoso architecture with Networking, Web, App, Data, and Monitoring tiers.
- **Output**: `diagram/diagrams/contoso_architecture.*`

### 3. Terraform Demo (Auto-parsed)
- **Source**: `terraform-demo/main.tf`
- **Resources**: 19 Azure resources
- **Output**: `diagrams/sc_demo_arch.*`

### 4. Bicep Lab01 (Bicep-parsed)
- **Source**: `avm-bicep-labs/labs/lab01/`
- **Scenario**: CMK with User-Assigned Identity
- **Output**: `diagrams/lab01_architecture.*`

### 5. IIS + SQL 3-Tier (ARM-parsed)
...
6. **Ensure dependencies are installed**: Critical step before running any script
